...
interface PhotoDao {
	...
	@Query("UPDATE photo SET syncState = :state WHERE photo_uuid = :uuid")
	suspend fun updateSyncStateReturningRows(
		uuid: String,
		state: SyncState,
	): Int
	...
	/**
	 * Count LIVE (non-deleted) photos with the given content_hash.
	 *
	 * Anti-dedup: used by [HashRegistry.softDelete] to check if any other Photo row
	 * still references this content_hash before tombstoning the registry entry.
	 * If count > 1, the hash is still in use by another photo — must NOT tombstone
	 * (would cause the remote original to be GC'd while a live photo still needs it).
	 *
	 * @since anti-dedup fix — reference count check before softDelete
	 */
	@Query("SELECT COUNT(*) FROM photo WHERE content_hash = :hash AND deleted_at = 0")
	suspend fun countLiveByContentHash(hash: String): Int
	 ...
	 */
	@Query(
		"SELECT * FROM photo WHERE content_hash = :hash AND photo_uuid != :excludeUuid AND deleted_at = 0 AND (vault_id = :vaultId OR vault_id IS NULL) LIMIT 1",
	)
	...
	suspend fun updateCanonicalUuid(
		uuid: String,
