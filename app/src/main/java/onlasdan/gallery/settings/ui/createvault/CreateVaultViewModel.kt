...
sealed interface CreateVaultUiEvent {
	...
	data class PasswordConfirmChanged(
		val value: String,
	) : CreateVaultUiEvent
...
class CreateVaultViewModel
	...
	constructor(
		...
		fun handleUiEvent(event: CreateVaultUiEvent) {
			when (event) {
				is CreateVaultUiEvent.PasswordChanged ->
					_uiState.update {
						it.copy(password = event.value, passwordAlreadyUnlocks = false, errorMessage = null)
					}
				is CreateVaultUiEvent.PasswordConfirmChanged ->
					_uiState.update {
		 ...
		 */
		fun strengthLabel(): Int =
			when (StrongPasswordPolicy.strength(_uiState.value.password)) {
