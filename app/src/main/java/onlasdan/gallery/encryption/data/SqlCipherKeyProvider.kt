...
class SqlCipherKeyProvider
	...
	constructor(
		...
		private val config: onlasdan.gallery.settings.data.Config,
	) {
		/**
		 ...
		 *
		 * F-ENC-001: If the Keystore fails (corrupt, hardware issue), this
		 * method catches the exception, sets [Config.keystoreFallbackActive]
		 * so the Settings UI can surface a warning, and delegates to
		 * [FallbackSqlCipherKeyProvider] (which stores the passphrase in
		 * plaintext SharedPreferences — NO security benefit over plaintext DB).
		 */
		fun getOrCreatePassphrase(): ByteArray {
			return try {
				...
				Timber.e(e, "CRITICAL: Android Keystore unavailable — SQLCipher is effectively plaintext")
