...
class SqlCipherMigrationHelper @Inject constructor(
    ...
    private fun runMigration(dbFile: File): Boolean {
        ...
        try {
            ...
            plaintextDb = net.sqlcipher.database.SQLiteDatabase.openOrCreateDatabase(
                dbFile.absolutePath,
     ...
     * @since v16 — Sprint 3 / TODO #6 SQLCipher
     */
    private fun copyVaultProtectionToBootstrap(
        plaintextDb: net.sqlcipher.database.SQLiteDatabase,
    ) {
        // Check if vault_protection table exists in the plaintext DB.
        val tableExists = plaintextDb.rawQuery(
        ...
        try {
            ...
            while (cursor.moveToNext()) {
                val bindArgs = Array(columnNames.size) { idx ->
