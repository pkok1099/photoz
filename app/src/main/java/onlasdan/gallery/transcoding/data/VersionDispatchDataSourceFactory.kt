...
class VersionDispatchDataSource(
	...
	override fun open(dataSpec: androidx.media3.datasource.DataSpec): Long {
		...
		val versionByte: Byte = try {
			if (!file.exists()) {
				// File doesn't exist yet (progressive download) — default to CBC
				// since existing videos are all CBC. New chunked-GCM videos will
				// have the file present before playback starts (they're local).
				0x02
			} else {
