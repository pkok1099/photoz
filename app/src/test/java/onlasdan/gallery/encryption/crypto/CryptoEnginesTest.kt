...
class CryptoEnginesTest {
	...
	/**
	 * F-ENC-024: HybridCryptoEngine now throws CorruptHeaderException for
	 * unrecognized version byte (was: return null — ambiguous).
	 */
	@Test
	fun `CBC engine throws for unrecognized version byte`() {
		val session = VaultSession(keyGen.generateVaultMasterKey())
		try {
			cbcEngine.createDecryptStream(ByteArrayInputStream(byteArrayOf(0x09)), session)
			throw AssertionError("Unrecognized version byte must throw, not return")
		} catch (e: onlasdan.gallery.encryption.domain.crypto.CorruptHeaderException) {
			// Expected
		}
	}
	...
	fun `GCM decrypt detects tampered ciphertext via per-chunk auth tag`() {
		...
		// F-ENC-011: ChunkedGcmInputStream now throws IOException on AEADBadTagException
		// (was: silently swallow as EOF -> readBytes() returned empty array).
		val stream = cbcEngine.createDecryptStream(ByteArrayInputStream(ciphertext), session)
		try {
			stream.readBytes()
			throw AssertionError("Tampered ciphertext must throw IOException, not return data")
		} catch (e: java.io.IOException) {
			// Expected - GCM auth tag verification failed
		} finally {
			stream.close()
		}
	}
