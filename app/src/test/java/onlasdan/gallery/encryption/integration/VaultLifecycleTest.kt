...
class VaultLifecycleTest {
	...
	private val changePasswordUseCase =
	 ...
	 */
	@Test
	fun `create vault and unlock returns consistent VMK`() =
		runTest {
	...
	fun `wrong password cannot unlock vault`() =
		runTest {
			val protection = handler.create(CreateRequest.Password(password))
	 ...
	 */
	@Test
	fun `change password re-encrypts VMK, new password unlocks vault with same VMK, old password fails`() =
		runTest {
	 ...
	 */
	@Test
	fun `migrate from 2xx vault is accessible with correct password and VMK equals original PBKDF2 key`() =
		runTest {
	 ...
	 */
	@Test
	fun `migrate from 1xx vault is accessible with correct password using fresh random VMK`() =
		runTest {
			every { mockConfig.legacyPasswordHash } returns bcryptHash
			every { mockConfig.legacyUserSalt } returns null
