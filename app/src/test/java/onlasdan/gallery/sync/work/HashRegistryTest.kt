...
class HashRegistryTest {
	 ...
	 */
	@Test
	fun `uploadToRemote then downloadAndCache round-trips entries through GCM and GZIP`() =
		runTest {
			val entries =
				listOf(
					HashRegistryEntry(
			...
			assertTrue(
				"On-wire bytes must include version prefix + entry count + at least one entry",
	 ...
	 */
	@Test
	fun `downloadAndCache with wrong VMK returns zero entries without throwing`() =
		runTest {
	 ...
	 */
	@Test
	fun `downloadAndCache returns zero when registry not on remote`() =
		runTest {
			coEvery { rcloneController.downloadFile(any(), any()) } returns
				Result.failure(
